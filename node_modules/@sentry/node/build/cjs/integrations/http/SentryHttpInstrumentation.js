   ...
   */
   _onOutgoingRequestFinish(request, response) {
    ...
    const shouldCreateBreadcrumb =
      typeof _ignoreOutgoingRequests === 'function' ? !_ignoreOutgoingRequests(getRequestUrl.getRequestUrl(request), options) : true;
  ...
  try {
    ...
    req.on = new Proxy(req.on, {
      apply: (target, thisArg, args) => {
        ...
        if (event === 'data') {
          debugBuild.DEBUG_BUILD && core$1.logger.log(INSTRUMENTATION_NAME, 'Handling request.on("data")');
